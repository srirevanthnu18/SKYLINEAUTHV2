/*
 * SKYLINE C# SDK - Pre-configured for {{APP_NAME}}
 * ===============================================
 * Credentials:
 * Name: {{APP_NAME}}
 * Owner ID: {{OWNER_ID}}
 ...
 * 
 * Usage:
 * var SKYLINEApp = new SKYLINE.api(
 *     name: "{{APP_NAME}}",
 *     ownerid: "{{OWNER_ID}}",
 *     secret: "{{APP_SECRET}}",
 *     version: "{{VERSION}}"
...
namespace SKYLINE
{
    public class api {
        ...
        /// <summary>
        /// Set up your application credentials in order to use SKYLINE
        /// </summary>
        ...
        public api(string name, string ownerid, string secret, string version) {
            if (ownerid.Length < 10 || secret.Length != 64) {
                Thread.Sleep(2000);
                error("Application not setup correctly. Please ensure your OwnerID and Secret are correct.");
                Environment.Exit(0);
            }
        ...
        bool initialized;
        /// <summary>
        /// Initializes the connection with SKYLINE in order to use any of the functions
        /// </summary>
        public void init() {
        ...
        /// <summary>
        /// Checks if SKYLINE is been Initalized
        /// </summary>
        public void CheckInit() {
            if (!initialized) {
                error("You must run the function SKYLINEApp.init(); first");
                Environment.Exit(0);
            }
        }
        ...
        /// <summary>
        /// Use Buttons from SKYLINE Customer Panel
        /// </summary>
        ...
        /// <summary>
        /// SKYLINE acts as proxy and downlods the file in a secure way
        /// </summary>
        ...
        private static string req(NameValueCollection post_data) {
            try {
                using (WebClient client = new WebClient()) {
                    ...
                    string url = "{{API_URL}}";
                    if (!url.EndsWith("/")) url += "/";
                    var raw_response = client.UploadValues(url, post_data);
                ...
                if (webex.Response != null) {
                    ...
                    switch (response.StatusCode) {
                        case (HttpStatusCode)429: // client hit our rate limit
                            ...
                            return "";
                        default:
                            error($"Connection failure. Status Code: {response.StatusCode}");
                            Environment.Exit(0);
                            return "";
                    }
                } else {
                    error($"Connection failure. Error: {webex.Message}");
                    Environment.Exit(0);
                    return "";
                }
            }
        ...
        private static bool assertSSL(object sender, X509Certificate certificate, X509Chain chain, SslPolicyErrors sslPolicyErrors) {
            // For private projects, we allow all certificates to avoid "Connection Failure" on non-standard setups.
            return true;
        }
